...
const AcceptanceCardForSuperAdmin = (props: IProps) => {
  const { acceptance } = props;
  const [isOpen, setIsOpen] = useState<boolean>(false);
  const [isModalApprovalOpen, setIsModalApprovalOpen] =
      ...
      <div className="flex flex-col">
        ...
        {acceptance?.acceptedForPublicationStatus === 1 ? (
          ...
          <>
            <Button
              className="hover:cursor-pointer hover:text-blue-500 gap-2 mt-1"
              onClick={() => setIsModalApprovalOpen(true)}
            >
...
export { AcceptanceCardForSuperAdmin };
