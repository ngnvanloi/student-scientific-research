            ...
            childrenTrigger={
              <div className="flex gap-2">
                <p className="font-semibold text-blue-700 text-base">
                  Nhận xét nghiệm thu trước đó
                </p>
              </div>
            }
            childrenContent={
              <div className="mt-3">
                {acceptance?.reviewAcceptances.map((item, index) => {
                  return (
                    <div key={index} className="border p-2 mb-1">
                      <p className="font-semibold">{item.organizer.name}</p>
                      <p className="text-xs">
                        Ngày đánh giá: {formatDate(item.createdAt || "")}
                      </p>
                      <p className="mt-2">Nội dung: {item.description}</p>
                      {(() => {
                        if (item.isAccepted === true) {
                          ...
                          return (
                            <p className="text-red-600">
                              Kết luận: Chưa đạt yêu cầu
                            </p>
                          );
                        } else {
                          return (
                            <p className="text-yellow-500">
                              Kết luận: Chờ đánh giá
                            </p>
                          );
                        }
                      })()}
