...
import { AcceptanceCardForSuperAdmin } from "../AcceptanceCard/AcceptanceCardForSuperAdmin";
...
const TrackOngoingProjectEvaluationsForSuperAdmin = () => {
  // lấy ra danh sách các sản phẩm nghiệm thu
  let params: ParamsGetListAcceptanceForAllRole = {
    index: 1,
    pageSize: 100,
  ...
  return (
    <div>
      <Alert
        message="Dưới đây là danh sách các sản phẩm nghiệm thu đã được phê duyệt ở cấp Khoa. Hãy tiến hành kiểm tra phê duyệt để công khai nghiệm lên hệ thống website"
        type="info"
        showIcon
        className="text-base mb-3 text-justify px-3 py-2"
      />
      <div className="mt-5">
        {listAcceptance?.data.items.map((item, index) => {
          return (
            <AcceptanceCardForSuperAdmin acceptance={item} key={item.id} />
          );
        })}
      </div>
    </div>
...
export {
  TrackOngoingProjectEvaluations,
  TrackOngoingProjectEvaluationsForSuperAdmin,
};
