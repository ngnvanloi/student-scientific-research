...
export type ParamsGetListPostForOrganizer = {
  index: number;
  pageSize: number;
  idSearch?: string;
  nameSearch?: string;
...
export const useGetListPostForOrganizer = (
...
export async function GetListPostForOrganizer(
  param: ParamsGetListPostForOrganizer
): Promise<IDataResponseFromAPI<IListDataResponseFromAPI<Post>>> {
  const baseUrl = `${process.env.NEXT_PUBLIC_COMMUNITY_BASE_URL}api/Post/organizer`;
  //
